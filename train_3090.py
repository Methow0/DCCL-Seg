...
from CE_Net import CE_Net_, Our_Net_V4_, Our_Net_V5_, My_CE_Net_,Our_CE_Net_,Full_CE_Net_
...
#    model = Our_CE_Net_()
    ...
    for epoch in range(opt.train['start_epoch'], opt.train['num_epochs']):
     ...
     # Calculate and log the duration for this epoch
        epoch_duration = end_time - start_time
        logger.info('Epoch [{:d}/{:d}] completed in {:.2f} seconds'.format(epoch + 1, opt.train['num_epochs'], epoch_duration))
...
def train(train_loader,model, optimizer, criterion, epoch):
    ...
    for i, sample in enumerate(train_loader):
        ...
        # loss_CE = 0.85*loss_CE + 0.1*loss_CE1 + 0.05*loss_CE2
        if opt.train['alpha'] != 0:
