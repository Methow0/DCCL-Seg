...
from options_CRAG import Options
...
parser.add_argument('--save-dir', type=str, default='./experiments/GlaS/newcrop/train_12-16', help='directory to save training results')
...
def main():
    ...
    nf_model= nn.DataParallel(nf_model)
    ...
    for epoch in range(opt.train['start_epoch'], opt.train['num_epochs']):
        ...
        train_results = train(train_loader, model, nf_model, optimizer, optimizer_nf, criterion, epoch)
...
def train(train_loader, model, nf_model,optimizer, optimizer_nf, criterion, epoch):
    ...
    for i, sample in enumerate(train_loader):
        ...
        output, out_de = model(input_var)
        b,c,h,w = out_de.size()  #pred_all, rep_all, fts_all
        total_n = int(b*h*w)
        out_de= out_de.detach().permute(0,2,3,1).reshape(total_n, c)

        z, log_jac_det = nf_model(out_de)
        label_nf = target_var.detach().clone().view(-1)
        nf_loss, ll, sldj = loss_flow(z, sldj=log_jac_det, y=label_nf )
