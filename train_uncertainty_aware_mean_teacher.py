...
def main():
    ...
    for epoch in range(opt.train['start_epoch'], opt.train['num_epochs']):
        ...
        train_results = train(train_loader, train_loader1, model, ema_model, optimizer, criterion, epoch)
...
def train(train_loader, train_loader1, model, ema_model, optimizer, criterion, epoch):
    ...
    for i, sample in enumerate(train_loader):
        ...
        cons_loss = consistency_loss(output_u, output_ema)
        T = 8
        volume_batch_r = input1[0].repeat(2, 1, 1, 1)
        stride = volume_batch_r.shape[0] // 2
        preds = torch.zeros([stride * T, 3, 512, 512]).cuda()
        for i in range(T // 2):
            ...
            with torch.no_grad():
                preds[2 * stride * i:2 * stride *
                                     (i + 1)] = ema_model(ema_inputs.cuda())
        preds = F.softmax(preds, dim=1)
        preds = preds.reshape(T, stride, 3, 512, 512)
        ...
        unsup_loss = torch.sum(
            mask * cons_loss) / (2 * torch.sum(mask) + 1e-16)
        ...
        if opt.train['alpha'] != 0:
            ...
            loss = loss_CE + opt.train['alpha'] * loss_var  + unsup_loss
