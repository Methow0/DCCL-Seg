...
def main():
    ...
    criterion_e =  torch.nn.CrossEntropyLoss().cuda()
    ...
    for epoch in range(opt.train['start_epoch'], opt.train['num_epochs']):
        ...
        train_results = train(train_loader, train_loader1, model, ema_model, optimizer, criterion,criterion_e, epoch)
...
def train(train_loader, train_loader1, model, ema_model, optimizer, criterion,criterion_e, epoch):
    ...
    for i, sample in enumerate(train_loader):
        ...
        # inputs_u1_noise = transforms_for_noise(input1[0].cuda(), 0.5)
        #
        # inputs_u1_noise, rot_mask, flip_mask = transforms_for_rot(inputs_u1_noise)

        # compute output

        output, output_u = model(input_var, input1[0].cuda())



        with torch.no_grad():
            output_ema = ema_model(input1[0].cuda())
        # output_ema = transforms_back_rot(output_ema, rot_mask, flip_mask)
        p = (torch.softmax(output_u, dim=1) + torch.softmax(output_ema, dim=1)) / 2
        pt = p ** (1 / 0.5)
        targets_u = pt / pt.sum(dim=1, keepdim=True)
        targets_u = targets_u[:, 1, :, :]
        targets_u = targets_u.detach()

        consistency_dist = 0.5 * criterion_e(output_u, targets_u.long()) + 0.5 * criterion_e(output_ema, targets_u.long())
        ...
        if opt.train['alpha'] != 0:
            ...
            loss = loss_CE + opt.train['alpha'] * loss_var  + consistency_dist
