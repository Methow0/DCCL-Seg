...
from options_PGland_semi import Options
...
def main():
    ...
    for x in ['train', 'val']:
        ...
        dir_list = [img_dir, target_dir1, target_dir]
        print(dir_list)
        if opt.dataset == 'PGlandSeg':
            post_fix = ['anno_label.png','anno_label.png']
        else:
...
def train(train_loader, train_loader1, model, ema_model, gta_model,optimizer, criterion, epoch):
    ...
    for i, sample in enumerate(train_loader):
        ...
        loss_CE1 = compute_unsupervised_loss_conf_weight(label_u_aug,75,output_u)
