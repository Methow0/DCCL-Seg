...
def main():
    ...
    DAN = FCDiscriminator(num_classes=3)
    DAN = DAN.cuda()
    ...
    DAN_optimizer = torch.optim.Adam(
    ...
    for epoch in range(opt.train['start_epoch'], opt.train['num_epochs']):
        ...
        train_results = train(train_loader, train_loader1, model,DAN, optimizer, DAN_optimizer, criterion, epoch)
...
def train(train_loader, train_loader1, model, DAN,optimizer, DAN_optimizer,criterion, epoch):
    ...
    for i, sample in enumerate(train_loader):
        ...
        DAN_target = torch.tensor([0] * 8).cuda()
        DAN_target[0:] = 1
        DAN.eval()
        ...
        DAN_outputs_u = DAN(output_u, input1[0].cuda())
        consistency_loss = F.cross_entropy(DAN_outputs_u, DAN_target.long())
        # print("consistency_loss:",consistency_loss)

        output_soft = torch.softmax(output, dim=1)
        DAN_outputs = DAN(output_soft, input_var)
        DAN_loss = F.cross_entropy(DAN_outputs, DAN_target.detach().long())
        # print("DAN_loss:",DAN_loss)
        ...
        if opt.train['alpha'] != 0:
            ...
            loss = loss_CE + opt.train['alpha'] * loss_var +  consistency_loss + DAN_loss
